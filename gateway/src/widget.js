...
(function () {
  ...
  function assignStyle(node, styles) {
    for (var k in styles) { if (styles.hasOwnProperty(k)) node.style[k] = styles[k]; }
  }
  ...
  var baseUrl = (cs && (cs.getAttribute('data-base') || cs.getAttribute('data-baseurl'))) || globalCfg.baseUrl || '';
  ...
  var label = (globalCfg.label || (cs && cs.getAttribute('data-label')) || window.WIDGET_CHAT_LABEL || 'Live Help');
  var logoUrl = (globalCfg.logo || (cs && cs.getAttribute('data-logo')) || window.WIDGET_LOGO_URL || '');
  var position = (globalCfg.position || (cs && cs.getAttribute('data-position')) || window.WIDGET_POSITION || '');
  ...
  try { sessionUuid = localStorage.getItem(STORAGE_KEY) || null; } catch (e) {}
  var VISITOR_KEY = 'arpchat.visitorId';
  var visitorId = null;
  try { visitorId = localStorage.getItem(VISITOR_KEY) || null; } catch (e) {}
  if (!visitorId) {
  ...
  bubble.type = 'button';
  bubble.innerHTML = '<span style="display:inline-block;width:10px;height:10px;border-radius:999px;background:#22c55e;margin-right:8px;vertical-align:middle"></span><span>' + label + '</span>';
  ...
  style.textContent =
    ...
    '.arpchat-bubble{position:fixed !important;bottom:16px;' + dockSide + 'z-index:2147483000;border:0;border-radius:999px;padding:10px 14px;font:600 14px/1.2 system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;color:' + colors.text + ';background:' + colors.panelBg + ';box-shadow:0 10px 24px rgba(0,0,0,.35);cursor:pointer;width:auto !important;max-width:calc(100vw - 32px)}' +
    '.arpchat-panel{position:fixed !important;bottom:72px;' + dockSide + 'width:320px !important;max-width:calc(100vw - 32px) !important;height:480px !important;max-height:calc(100vh - 96px) !important;z-index:2147483001;border-radius:16px;overflow:hidden;display:none;box-shadow:0 20px 50px rgba(0,0,0,.45);background:' + colors.bg + ';color:' + colors.text + ';font:14px/1.35 system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial}' +
    '.arpchat-header{position:relative !important;display:flex;align-items:center !important;justify-content:flex-start;padding:12px 12px;background:' + colors.panelBg + ';border-bottom:1px solid rgba(255,255,255,.08);gap:10px}' +
    ...
    '.arpchat-close{position:absolute !important;top:6px;right:8px;left:auto;margin:0;margin-left:auto;padding:0;border:0;background:transparent;color:' + colors.muted + ';font-size:22px;line-height:1;cursor:pointer;min-width:0;min-height:0;width:24px;height:24px;display:inline-flex;align-items:center;justify-content:center}' +
    '.arpchat-status{padding:8px 12px;font-size:12px;color:' + colors.muted + ';border-bottom:1px solid rgba(255,255,255,.06)}' +
    ...
    '.arpchat-row.visitor .arpchat-bubblemsg{background:' + colors.bubbleVisitor + '}' +
    '.arpchat-row.agent .arpchat-bubblemsg{background:' + colors.bubbleAgent + '}' +
    '.arpchat-row.system .arpchat-bubblemsg{background:rgba(255,255,255,.06);color:' + colors.muted + '}' +
    '.arpchat-form{position:relative;display:flex;gap:8px;padding:10px 12px;background:' + colors.panelBg + ';border-top:1px solid rgba(255,255,255,.08);box-sizing:border-box;align-items:center;flex-wrap:nowrap}' +
    ...
    '.arpchat-emoji-toggle{border:1px solid rgba(255,255,255,.12);background:transparent;color:' + colors.text + ';border-radius:8px;padding:6px 8px;cursor:pointer;font-size:14px;line-height:1;display:inline-flex;align-items:center;justify-content:center}' +
    ...
    '.arpchat-emoji-panel button{border:1px solid rgba(255,255,255,.08);background:transparent;color:' + colors.text + ';border-radius:8px;padding:6px 0;cursor:pointer;font-size:16px;line-height:1}' +
    '.arpchat-input{flex:1 1 auto !important;width:auto !important;min-width:0 !important;max-width:100%;display:block !important;visibility:visible !important;opacity:1 !important;padding:10px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.10);background:rgba(0,0,0,.15);color:' + colors.text + ';outline:none;box-sizing:border-box;font-size:14px;line-height:1.4}' +
    '.arpchat-send{flex:0 0 auto !important;width:auto !important;max-width:90px !important;min-width:0 !important;padding:8px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.14);background:transparent;color:' + colors.text + ';cursor:pointer;align-self:auto;font-size:13px;line-height:1.1;white-space:nowrap;display:inline-flex;align-items:center;justify-content:center}' +
    '.arpchat-end{flex:0 0 auto !important;width:auto !important;max-width:90px !important;min-width:0 !important;padding:8px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.14);background:transparent;color:' + colors.text + ';cursor:pointer;align-self:auto;font-size:13px;line-height:1.1;white-space:nowrap;opacity:.85;display:inline-flex;align-items:center;justify-content:center}' +
    '.arpchat-send:hover{border-color:' + colors.accent + ';color:' + colors.accent + '}';
  ...
  function startOrResume(cb) {
    if (sessionUuid) {
      xhrJson('POST', baseUrl + '/api/session/resume', { sessionUuid: sessionUuid, entryUrl: location.href, referrer: document.referrer || '', visitorId: visitorId }, function (ok, data) {
  ...
  function startFresh(cb) {
    ...
    var payload = { entryUrl: location.href, referrer: document.referrer || '', visitorId: visitorId };
    ...
    xhrJson('POST', baseUrl + endpoint, payload, function (ok, data) {
      ...
      try { localStorage.setItem(STORAGE_KEY, sessionUuid); } catch (e) {}
      trackPage();
      restoreHistory();
      cb(true, sessionUuid);
    });
  ...
  function connectWs() {
    ...
    startOrResume(function (ok, sid) {
      ...
      ws.onopen = function () {
        ...
        try { ws.send(JSON.stringify({ type: 'hello', sessionUuid: sid })); } catch (e) {}
      };
  ...
  function sendVisitor(text) {
    ...
    try { ws.send(JSON.stringify({ type: 'visitor_message', text: t })); } catch (e) {}
  }
  ...
  function endChat() {
    ...
    xhrJson('POST', baseUrl + '/api/session/close', { sessionUuid: sessionUuid }, function () {
      try { localStorage.removeItem(STORAGE_KEY); } catch (e) {}
  ...
  sendBtn.addEventListener('click', function () { sendVisitor(input.value); });
  endBtn.addEventListener('click', endChat);
  input.addEventListener('keydown', function (e) {
    e = e || window.event;
    if (e.keyCode === 13) { e.preventDefault(); sendVisitor(input.value); }
  });
  var emojiList = ['😀','😁','😂','🤣','😃','😄','😅','😆','😉','😊','😇','🙂','🙃','😍','🥰','😘','😗','😙','😚','😋','😜','😝','😛','🤪','🤗','🤔','🤨','😐','😑','😶','🙄','😏','😒','😞','😔','😟','😕','🙁','☹️','😣','😖','😫','😩','🥺','😢','😭','😤','😠','😡','🤬','🤯','😳','🥵','🥶','😱','😨','😰','😥','😓','🤭','🤫','🤥','😶‍🌫️','😴','🤤','😪','😵','😵‍💫','🤐','🥴','🤒','🤕','🤢','🤮','🤧','😷','🤠','🥳','😎','🤓','🧐','😕','😟','🙁','☹️','👍','👎','👊','✊','🤝','🙏','👏','🙌','🫶','💪','❤️','🧡','💛','💚','💙','💜','🖤','🤍','🤎','💔','✨','🎉','🎊','🔥','⭐','🌟','💫','☀️','🌈','⚡','💬','✅','❌'];
  if (emojiPanel) {
    var emojiHtml = '';
    for (var i = 0; i < emojiList.length; i += 1) {
      emojiHtml += '<button type="button" data-emoji="' + emojiList[i] + '">' + emojiList[i] + '</button>';
    }
