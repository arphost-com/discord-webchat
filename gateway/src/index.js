...
async function main() {
  logger.info('[BOOT] starting', { node: process.version, env: process.env.NODE_ENV || 'development' });
  ...
  if (retentionDays > 0) {
    try {
      ...
      logger.warn('[TRACKING] cleanup failed', { err: String(e?.message || e) });
    }
    setInterval(async () => {
      try {
...
main().catch((err) => {
  logger.error('[BOOT] fatal', { err: (err && err.stack) ? err.stack : String(err) });
  process.exitCode = 1;
});
