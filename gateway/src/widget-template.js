...
(function () {
  ...
  var bubble = el("button", { class: "arpchat-bubble", type: "button", "aria-label": "Open chat" });
  ...
  var closeBtn = el("button", { class: "arpchat-close", type: "button", "aria-label": "Close chat", text: "×" });
  ...
  var nameInput = el("input", { class: "arpchat-input", placeholder: "Your name" });
  var emailInput = el("input", { class: "arpchat-input", placeholder: "Your email (optional)" });
  var startBtn = el("button", { class: "arpchat-start", type: "button", text: "Start chat" });
  pre.appendChild(nameInput);
  pre.appendChild(emailInput);
  pre.appendChild(startBtn);

  var composer = el("div", { class: "arpchat-composer" });
  var emojiToggle = el("button", { class: "arpchat-emoji-toggle", type: "button", text: "😊" });
  var emojiPanel = el("div", { class: "arpchat-emoji-panel" });
  var msgInput = el("input", { class: "arpchat-msg", placeholder: "Type a message..." });
  var sendBtn = el("button", { class: "arpchat-send", type: "button", text: "Send" });
  var endBtn = el("button", { class: "arpchat-end", type: "button", text: "End" });
  ...
  function startSession() {
    ...
    http("POST", "/api/session/start-guest", payload, function (ok, status, text) {
      if (!ok) {
  ...
  function sendMsg() {
    ...
    if (ws && ws.readyState === 1) {
      ws.send(JSON.stringify({ t: "visitor_message", sessionUuid: sessionUuid, text: text }));
    } else {
  ...
  function endChat() {
    ...
    http("POST", "/api/session/close", { sessionUuid: sessionUuid }, function () {
