...
function discord_webchat_get_option($key, $default = '')
{
    $opts = get_option('discord_webchat_options', []);
    return isset($opts[$key]) ? $opts[$key] : $default;
}

function discord_webchat_register_settings()
{
    register_setting('discord_webchat', 'discord_webchat_options');
}
add_action('admin_init', 'discord_webchat_register_settings');

function discord_webchat_menu()
{
    add_menu_page('Discord WebChat', 'Discord WebChat', 'manage_options', 'discord-webchat', 'discord_webchat_admin_page', 'dashicons-format-chat');
}
add_action('admin_menu', 'discord_webchat_menu');

function discord_webchat_admin_page()
{
    $tab = isset($_GET['tab']) ? sanitize_text_field($_GET['tab']) : 'settings';
    echo '<div class="wrap"><h1>Discord WebChat</h1>';
    echo '<nav class="nav-tab-wrapper">';
    echo '<a class="nav-tab ' . ($tab==='settings'?'nav-tab-active':'') . '" href="?page=discord-webchat&tab=settings">Settings</a>';
    echo '<a class="nav-tab ' . ($tab==='dashboard'?'nav-tab-active':'') . '" href="?page=discord-webchat&tab=dashboard">Dashboard</a>';
    echo '</nav>';

    if ($tab === 'dashboard') {
        discord_webchat_render_dashboard();
    } else {
        discord_webchat_render_settings();
    }
    echo '</div>';
}

function discord_webchat_render_settings()
{
    $opts = get_option('discord_webchat_options', []);
    $gateway = esc_attr($opts['gateway_baseurl'] ?? '');
    ...
    echo '<tr><th scope="row">Gateway Base URL</th><td><input type="text" name="discord_webchat_options[gateway_baseurl]" value="'.$gateway.'" class="regular-text" placeholder="https://your-gateway.example.com"></td></tr>';
    echo '<tr><th scope="row">Admin API Key</th><td><input type="password" name="discord_webchat_options[admin_api_key]" value="'.$adminKey.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Google Maps API Key</th><td><input type="text" name="discord_webchat_options[google_maps_api_key]" value="'.$mapsKey.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Inject Widget</th><td><label><input type="checkbox" name="discord_webchat_options[enable_widget]" value="1" '.checked($enableWidget, true, false).'> Enable widget on all pages</label></td></tr>';
    echo '<tr><th scope="row">Widget Position</th><td><select name="discord_webchat_options[widget_position]">';
    ...
    echo '</select></td></tr>';
    echo '<tr><th scope="row">Widget Label</th><td><input type="text" name="discord_webchat_options[widget_label]" value="'.$label.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Logo URL</th><td><input type="text" name="discord_webchat_options[widget_logo_url]" value="'.$logoUrl.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Accent</th><td><input type="text" name="discord_webchat_options[widget_accent]" value="'.$accent.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Background</th><td><input type="text" name="discord_webchat_options[widget_bg]" value="'.$bg.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Panel Background</th><td><input type="text" name="discord_webchat_options[widget_panel_bg]" value="'.$panelBg.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Text Color</th><td><input type="text" name="discord_webchat_options[widget_text]" value="'.$text.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Muted Color</th><td><input type="text" name="discord_webchat_options[widget_muted]" value="'.$muted.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Visitor Bubble</th><td><input type="text" name="discord_webchat_options[widget_bubble_visitor]" value="'.$bubbleVisitor.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Agent Bubble</th><td><input type="text" name="discord_webchat_options[widget_bubble_agent]" value="'.$bubbleAgent.'" class="regular-text"></td></tr>';
    echo '<tr><th scope="row">Widget Error Color</th><td><input type="text" name="discord_webchat_options[widget_error]" value="'.$error.'" class="regular-text"></td></tr>';
    echo '</table>';
    submit_button();
    echo '</form>';
}

function discord_webchat_api_get($path)
{
    $base = rtrim(discord_webchat_get_option('gateway_baseurl', ''), '/');
    $key = discord_webchat_get_option('admin_api_key', '');
    if ($base === '' || $key === '') {
...
function discord_webchat_render_dashboard()
{
    $range = isset($_GET['range']) ? preg_replace('/[^a-z0-9]/i', '', $_GET['range']) : 'all';
    if ($range === '') $range = 'all';
    $resp = discord_webchat_api_get('/api/admin/sessions?limit=200&range=' . urlencode($range));
    if (!$resp['ok']) {
    ...
    $mapKey = discord_webchat_get_option('google_maps_api_key', '');
    ...
    if ($mapKey !== '') {
        ...
        echo '<h2>Map</h2>';
        echo '<div id="dwc-map" style="width:100%;height:420px;"></div>';
        ...
        echo 'var map = new google.maps.Map(document.getElementById("dwc-map"), {zoom:2, center:center});';
...
function discord_webchat_inject_widget()
{
    $opts = get_option('discord_webchat_options', []);
...
add_action('wp_footer', 'discord_webchat_inject_widget');
