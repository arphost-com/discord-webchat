...
import { startGuestSession, startClientSession, resumeSession, handleVisitorMessage, handleHello } from './sessions.js';
import {
  insertPageView,
...
export function setupRoutes(app, deps = {}) {
  ...
  const tokenMaxAgeSeconds = Number(process.env.WIDGET_TOKEN_MAX_AGE_SECONDS || 300);
  ...
  const corsMiddleware = cors({
    ...
    allowedHeaders: ['Content-Type','X-Widget-Token','X-Widget-Signature','X-Widget-Timestamp'],
    maxAge: 600,
  });
  ...
  app.get('/widget.js', (req, res) => {
    if (widgetJsCacheError) {
      logger?.error?.('[HTTP] widget.js cache error', { err: widgetJsCacheError });
  ...
  app.post('/api/session/start-guest', express.json({ limit: '64kb' }), async (req, res) => {
    try {
      const out = await startGuestSession({ pool, discord, logger, body: req.body || {} });
      safeJson(res, 200, { ok: true, ...out, publicBaseUrl });
    } catch (e) {
      logger?.error?.('[HTTP] start-guest failed', { err: String(e?.stack || e) });
      safeJson(res, 500, { ok: false, error: 'start_failed' });
    }
  });

  app.post('/api/session/start-client', express.json({ limit: '64kb' }), async (req, res) => {
    try {
      const payload = verifyWidgetToken(req.body?.token, widgetSecret, tokenMaxAgeSeconds);
      const out = await startClientSession({ pool, discord, logger, body: req.body || {}, payload });
      safeJson(res, 200, { ok: true, ...out, publicBaseUrl });
    } catch (e) {
      logger?.warn?.('[HTTP] start-client failed', { err: String(e?.message || e), token: summarizeToken(req.body?.token) });
      safeJson(res, 401, { ok: false, error: 'invalid_token' });
    }
  });

  app.post('/api/session/resume', express.json({ limit: '64kb' }), async (req, res) => {
    try {
      const out = await resumeSession({ pool, discord, logger, body: req.body || {} });
      safeJson(res, 200, { ok: true, ...out, publicBaseUrl });
    } catch (e) {
  ...
  app.post('/api/track/page', express.json({ limit: '64kb' }), async (req, res) => {
    try {
      const body = req.body || {};
      const url = String(body.url || '').slice(0, 2048);
      if (!url) return safeJson(res, 400, { ok: false, error: 'missing_url' });
      ...
      logger?.warn?.('[HTTP] track page failed', { err: String(e?.message || e) });
      safeJson(res, 500, { ok: false, error: 'track_failed' });
    }
  });

  app.post('/api/session/close', express.json({ limit: '32kb' }), async (req, res) => {
    try {
      const sessionUuid = String(req.body?.sessionUuid || '').trim();
      if (!sessionUuid) return safeJson(res, 400, { ok: false, error: 'missing_session' });
      const sess = await getSession(pool, sessionUuid);
      if (!sess) return safeJson(res, 404, { ok: false, error: 'not_found' });
      await insertMessage(pool, { sessionUuid, direction: 'system', authorName: 'System', body: 'Chat closed by visitor.' });
      if (discord?.archiveThread && sess.discord_thread_id) {
        await discord.archiveThread(String(sess.discord_thread_id));
      }
      safeJson(res, 200, { ok: true });
    } catch (e) {
      logger?.warn?.('[HTTP] close session failed', { err: String(e?.message || e) });
  ...
  if (wsHub?.onVisitorMessage) {
    wsHub.onVisitorMessage(async ({ ws, sessionUuid, text }) => {
      await handleVisitorMessage({ ws, sessionUuid, text, pool, discord, logger });
    });
  }

  function requireAdmin(req, res, next) {
    if (!adminApiKey) return safeJson(res, 403, { ok: false, error: 'admin_disabled' });
  ...
  app.get('/api/admin/sessions', requireAdmin, async (req, res) => {
    try {
      ...
      logger?.warn?.('[HTTP] admin sessions failed', { err: String(e?.message || e) });
  ...
  app.get('/api/admin/session/:sessionUuid', requireAdmin, async (req, res) => {
    try {
      ...
      if (!session) return safeJson(res, 404, { ok: false, error: 'not_found' });
      const messages = await getTranscript(pool, sessionUuid);
      const range = req.query?.range;
      const days = parseRangeDays(range);
      const pageViews = await getPageViewsForSession(pool, sessionUuid, req.query?.limit, days);
      safeJson(res, 200, { ok: true, session, messages, pageViews, range: range || 'all' });
    } catch (e) {
      logger?.warn?.('[HTTP] admin session failed', { err: String(e?.message || e) });
      safeJson(res, 500, { ok: false, error: 'admin_failed' });
    }
  });
  ...
  app.get('/api/admin/pageviews', requireAdmin, async (req, res) => {
    try {
      ...
      logger?.warn?.('[HTTP] admin pageviews failed', { err: String(e?.message || e) });
      safeJson(res, 500, { ok: false, error: 'admin_failed' });
    }
  });


  logger?.info?.('[ROUTES] ready', { allowedOriginsCount: allowedOrigins.length });
}
